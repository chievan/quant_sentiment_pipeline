# 使用Claude模型的量化舆情流水线
# Clear environment proxies to prevent proxy blockages on the server
import os
for key in ['HTTP_PROXY', 'HTTPS_PROXY', 'http_proxy', 'https_proxy', 'ALL_PROXY', 'all_proxy']:
    if key in os.environ:
        del os.environ[key]

import json
import pandas as pd
from datetime import datetime

# Add local path to import submodules
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from utils.pdf_parser import PDFParser
from ingestion.rss_scraper import RSSScraper
# 移除DolphinDB数据库依赖: from ingestion.ddb_extractor import DDBExtractor
from engine.lexicon_model import LexiconModel
from engine.bert_model import BERTModel
from engine.claude_model import ClaudeModel
from engine.fof_attribution import FOFAttributionEngine
from utils.send_email import send_email
from utils.feishu_notify import send_feishu_report

def load_settings():
    config_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
        "config", 
        "settings.json"
    )
    try:
        with open(config_path, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        print(f"[Pipeline] Failed to load configuration: {e}")
        # Default fallback
        return {
            "rsshub": {"base_url": "http://106.54.219.69:1200", "timeout": 15},
            "pipeline": {
                "output_dir": "/Users/chievan/Documents/projects/quant_sentiment_pipeline/data/factors",
                "pdf_cache_file": "/Users/chievan/Documents/projects/quant_sentiment_pipeline/data/pdf_cache.json",
                "articles_limit_per_feed": 20
            }
        }

def main():
    print(f"============================================================")
    print(f"🚀 STARTING CLAUDE-BASED QUANT SENTIMENT PIPELINE RUN")
    print(f"⏰ Execution Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"💡 已移除DolphinDB数据库依赖，仅使用公开RSS数据源")
    print(f"============================================================")
    
    # 1. Load configuration
    settings = load_settings()
    pipe_cfg = settings["pipeline"]
    rss_cfg = settings["rsshub"]
    
    # Resolve output_dir dynamically if the local absolute path doesn't exist
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_dir = pipe_cfg["output_dir"]
    if not os.path.isabs(output_dir) or not os.path.exists(os.path.dirname(output_dir)):
        output_dir = os.path.join(project_root, "data", "factors")
        
    pdf_cache_file = pipe_cfg["pdf_cache_file"]
    if not os.path.isabs(pdf_cache_file) or not os.path.exists(os.path.dirname(pdf_cache_file)):
        pdf_cache_file = os.path.join(project_root, "data", "pdf_cache.json")
        
    # Ensure directories exist
    os.makedirs(output_dir, exist_ok=True)
    
    # 2. Initialize engines
    pdf_parser = PDFParser(cache_path=pdf_cache_file)
    rss_scraper = RSSScraper(base_url=rss_cfg["base_url"], timeout=rss_cfg["timeout"])
    # 移除DolphinDB数据库连接 - 仅使用公开RSS数据源
    
    lexicon_engine = LexiconModel()
    # Try loading cached models; if they are not cached, it will fall back gracefully to Lexicon
    bert_engine = BERTModel(local_files_only=True)
    # Use Claude model instead of Gemini
    claude_engine = ClaudeModel(mock_mode=False)  # 使用真实分析能力
    fof_engine = FOFAttributionEngine()
    
    # 3. Aggregating news corpus
    all_news = []
    
    # WeChat articles (removed DDB dependency)
    # 已移除数据库中的微信文章采集
    
    # RSS Feeds
    rss_articles = rss_scraper.fetch_all(pdf_parser=pdf_parser, limit=pipe_cfg["articles_limit_per_feed"])
    rss_articles = [a for a in rss_articles if fof_engine.clean_text(f"{a['title']} {a['summary']}")]
    all_news.extend(rss_articles)
    
    total_agg = len(all_news)
    print(f"\n[Pipeline] Ingestion completed. Total articles: {total_agg}")
    
    if total_agg == 0:
        print("[Pipeline] No articles found. Pipeline exiting.")
        return
        
    # 4. Scoring loop
    print("\n[Pipeline] Running scoring models...")
    scored_records = []
    
    for idx, item in enumerate(all_news):
        text = f"{item['title']} {item['summary']}"
        
        # Scoring with lexicon
        score_lexicon, channel_lexicon = lexicon_engine.score(text)
        
        # Scoring with BERT
        score_bert, channel_bert = bert_engine.score(text)
        
        # Scoring with Claude
        score_claude, channel_claude = claude_engine.score(text)
        
        # Attribution with FOF
        attributions = fof_engine.attribute_article(text)
        fof_strategy = ",".join([code for code, kws in attributions])
        fof_keywords = ",".join(list(set([kw for code, kws in attributions for kw in kws])))
        
        # Format record
        record = {
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "pub_date": item["pubDate"],
            "source": item["source"],
            "title": item["title"],
            "link": item["link"],
            "score_finnlp": score_lexicon,
            "channel_finnlp": channel_lexicon,
            "score_finbert": score_bert if score_bert is not None else 0.0,
            "channel_finbert": channel_bert,
            "score_claude": score_claude,
            "channel_claude": channel_claude,
            "fof_strategy": fof_strategy,
            "fof_keywords": fof_keywords
        }
        scored_records.append(record)
        
        if (idx + 1) % 50 == 0 or (idx + 1) == total_agg:
            print(f"  Scored {idx + 1}/{total_agg} articles...")
            
    # 5. Save results to monthly CSV file
    df = pd.DataFrame(scored_records)
    
    file_name = f"sentiment_factors_{datetime.now().strftime('%Y%m')}.csv"
    output_path = os.path.join(output_dir, file_name)
    
    # Append if exists, otherwise write header
    if os.path.exists(output_path):
        try:
            df.to_csv(output_path, mode='a', header=False, index=False, encoding='utf-8-sig')
            print(f"\n[Pipeline] Appended results to existing file: {output_path}")
        except Exception as e:
            print(f"\n[Pipeline] Failed to append, writing new file: {e}")
            df.to_csv(output_path, index=False, encoding='utf-8-sig')
    else:
        df.to_csv(output_path, index=False, encoding='utf-8-sig')
        print(f"\n[Pipeline] Generated new factors CSV file: {output_path}")
    
    # 6. 统计情感分布
    pos_count = len(df[df['score_claude'] > 0.1])
    neg_count = len(df[df['score_claude'] < -0.1])
    neu_count = len(df[(df['score_claude'] >= -0.1) & (df['score_claude'] <= 0.1)])
    
    # 获取top利好和利空文章
    top_pos = df[df['score_claude'] > 0.3].sort_values('score_claude', ascending=False).head(3).to_dict('records')
    top_neg = df[df['score_claude'] < -0.3].sort_values('score_claude', ascending=True).head(3).to_dict('records')
    
    # 提取热门关键词
    all_keywords = []
    for _, row in df.iterrows():
        if pd.notna(row['fof_keywords']) and row['fof_keywords']:
            all_keywords.extend([kw.strip() for kw in row['fof_keywords'].split(',') if kw.strip()])
    
    # 统计关键词频率
    from collections import Counter
    keyword_counter = Counter(all_keywords)
    hot_keywords = [kw for kw, cnt in keyword_counter.most_common(10)]
    
    # Delegate to the latest DH Brief generator (Part A Key Events table + Part B Top 5 Strategy Factors + Email/Feishu push)
    from generate_dh_brief import main as generate_dh_brief_main
    generate_dh_brief_main()

if __name__ == "__main__":
    main()
